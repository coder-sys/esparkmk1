        ...
        try{
          let ut = await fetch(`http://35.193.47.255/get_user_type/${firstname_google}`)
                  ...
                  if(api['data']!='username not found'){
                    setAccess("Granted")
					console.log(ut.data)
                    window.location.replace('https://e-spark-prod.ecsbeats.repl.co/homepage/'+firstname_google+'/'+ut.data)
                  }
                  else{
                    alert('Username not found')
                  }
								        ...
								        try{
                                            ...
                                            if(password == api["data"]){
                                              ...
                                              let ut = await fetch(`http://35.193.47.255/get_user_type/${name}`)
                                                        ut = await ut.json()
                                                        console.log(ut.data)
														window.location.replace('https://e-spark-prod.ecsbeats.repl.co/homepage/'+name+"/"+ut.data)
